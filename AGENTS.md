...
- **Where to find pi-local workflow rules**: See `pi/AGENTS.md` and `docs/agents/norpiv.md` for repository-specific workflow, hats, and naming conventions. For guidance on durable documentation edits, see `pi/skills/norpiv/update-docs/SKILL.md`.
...
- Validation: A lightweight, print-only validation script is available at `pi/scripts/norpiv/validate_active_task.sh` to help inspect and remediate inconsistent state without making automatic changes.
