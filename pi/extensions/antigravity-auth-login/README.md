...
| --- | --- | --- |
| Latest Pi (`0.72.1` observed during planning) | OAuth + model registration work; response path still under investigation | Local extension starts, provider registers, proxy can run |
...
Then select an `antigravity/...` model and use `/login` if needed.
